...
var time_since_last_in_obstacle := INF

func _physics_process(delta: float) -> void:
	
	time_since_last_in_obstacle += delta
	
	if Accessor.player.state.drag > Accessor.player.open_air_properties.drag:
		time_since_last_in_obstacle = 0.0
	
	var distance_traveled := Accessor.player.position.\
		distance_to(last_shadow_position)
	if distance_traveled > SHADOW_DIAMETER && time_since_last_in_obstacle < .5:
